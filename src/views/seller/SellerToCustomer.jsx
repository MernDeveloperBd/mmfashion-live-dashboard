// src/pages/Chat/SellerToCustomer.jsx
...
const SellerToCustomer = () => {
  ...
  // Admin notification handling kept as-is
  useEffect(() => {
        ...
        <div className="flex w-full h-full relative">
          ...
          <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
            <div className="w-full h-[calc(100vh-177px)] bg-[#252b3b] md:bg-transparent overflow-y-auto p-1">
              ...
              {customers?.map((c, i) => (
                <Link key={i} to={`/seller/dashboard/chat-customer/${c.fdId}`} className="h-[60px] flex justify-start items-center gap-2 text-white bg-slate-700 px-2 py-2 rounded-sm cursor-pointer mb-1">
                  <div className="relative">
                    ...
                    />
                    {
                      activeCustomer.some(ac => ac.customerId === c.fdId)
                ...
                <div className="flex justify-start items-center gap-3">
                  <div className="relative">
                    <img
                      ...
                      src={currentCustomer?.image || customerFallback}
                      alt={currentCustomer?.name || 'customer'}
                      onError={(e) => { e.currentTarget.src = customerFallback; }}
                    />
                    {
                      activeCustomer.some(ac => ac.customerId === currentCustomer._id)
                        ? <span className="absolute right-0 bottom-0 w-[10px] h-[10px] rounded-full bg-green-500 ring-2 ring-slate-800" />
                        : <span className="absolute right-0 bottom-0 w-[10px] h-[10px] rounded-full bg-gray-300 ring-2 ring-slate-800" />
                    }
                  </div>
                  <h2 className="text-base font-semibold text-white">{currentCustomer.name}</h2>
                </div>
              )}
